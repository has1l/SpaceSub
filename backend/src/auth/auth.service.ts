...
export class AuthService {
  ...
  private async exchangeCodeForToken(code: string) {
    ...
    this.logger.log(
      `[AUTH:token-exchange] env=YANDEX_CLIENT_ID, client_id=${clientId}, redirect_uri=${redirectUri}`,
    );
    ...
    try {
      ...
      this.logger.log('[AUTH:token-exchange] Yandex token exchange successful');
